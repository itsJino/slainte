...
# Returns a list of splits, each ending with the separator
...
# Chunks are split recursively by different methods
# If a chunk is larger than the given size, it is split recursively
# If a chunk is still larger after all methods are applied, it is split by words
# Chunks are then merged, with an overlap of a given size
class TextSplitter:
